// src/pages/Esteira/EsteiraRegistration.tsx
import React, { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { EsteiraActivity } from '../../types/activities.types';
import { workoutService } from '../../services/supabase.service';

const EsteiraRegistration: React.FC = () => {
  const navigate = useNavigate();

  const [duration, setDuration] = useState('');
  const [heartRate, setHeartRate] = useState('');
  const [calories, setCalories] = useState('');
  const [distance, setDistance] = useState('');
  const [notes, setNotes] = useState('');
  const [isLoading, setIsLoading] = useState(false);

  const quickDistances = [1000, 2000, 3000, 5000, 10000];
  const quickDurations = [15, 30, 45, 60];

  // Calcular velocidade média (km/h)
  const calculateAvgSpeed = (): number => {
    if (!distance || !duration) return 0;
    const distanceKm = parseFloat(distance) / 1000;
    const durationHours = parseFloat(duration) / 60;
    return distanceKm / durationHours;
  };

  // Calcular pace (min/km)
  const calculatePace = (): number => {
    if (!distance || !duration) return 0;
    const distanceKm = parseFloat(distance) / 1000;
    const durationMin = parseFloat(duration);
    return durationMin / distanceKm;
  };

  const handleSubmit = async () => {
    if (!duration || !heartRate || !calories || !distance) {
      return;
    }

    const avgSpeed = calculateAvgSpeed();

    const activity: EsteiraActivity = {
      id: `esteira-${Date.now()}`,
      type: 'esteira',
      date: new Date(),
      duration: parseFloat(duration) * 60, // converter para segundos
      calories: parseInt(calories),
      heartRate: parseInt(heartRate),
      distance: parseInt(distance),
      avgSpeed: avgSpeed, // ✅ Velocidade média salva
      notes: notes || undefined
    };

    setIsLoading(true);
    
    try {
      await workoutService.save({
        type: 'esteira',
        date: new Date(),
        duration: parseFloat(duration) * 60,
        calories: parseInt(calories),
        heart_rate: parseInt(heartRate),
        details: {
          distance: parseInt(distance),
          avgSpeed: avgSpeed // ✅ Velocidade média no detalhe
        },
        notes: notes || undefined
      });
      
      await sendTelegramReport(activity);
      
      navigate('/');
      
    } catch (error) {
      console.error('Erro:', error);
    } finally {
      setIsLoading(false);
    }
  };

  const sendTelegramReport = async (activity: EsteiraActivity): Promise<boolean> => {
    try {
      const formatDuration = (seconds: number): string => {
        const mins = Math.floor(seconds / 60);
        const secs = seconds % 60;
        return `${mins}:${secs.toString().padStart(2, '0')}`;
      };

      const distanceKm = (activity.distance / 1000).toFixed(2);
      const pace = (activity.duration / 60 / (activity.distance / 1000)).toFixed(2);
      const avgSpeed = activity.avgSpeed?.toFixed(1) || '0.0';

      const message = `
🚶‍♂️ *RELATÓRIO DE ESTEIRA / CAMINHADA* 🚶‍♀️

📅 *Data:* ${activity.date.toLocaleDateString('pt-BR')}
⏰ *Hora:* ${activity.date.toLocaleTimeString('pt-BR')}
⏱️ *Duração:* ${formatDuration(activity.duration)}

📊 *MÉTRICAS:*
├ 📏 Distância: ${distanceKm} km
├ 💓 FC Média: ${activity.heartRate} bpm
├ 🔥 Calorias: ${activity.calories} kcal
├ 🚀 Velocidade Média: ${avgSpeed} km/h
└ ⏱️ Pace: ${pace} min/km

📈 *ANÁLISE:*
├ ⚡ Intensidade: ${activity.heartRate < 120 ? 'Leve' : activity.heartRate < 150 ? 'Moderada' : 'Alta'}
└ 🎯 Eficiência: ${(activity.calories / (activity.duration / 60)).toFixed(1)} kcal/min

${activity.notes ? `📝 *OBSERVAÇÕES:*\n${activity.notes}\n` : ''}

💪 *PRÓXIMOS PASSOS:*
• Manter ritmo constante
• Aumentar distância gradualmente
• Controlar respiração

#Esteira #Caminhada #Cardio
      `.trim();

      const TELEGRAM_BOT_TOKEN = process.env.REACT_APP_TELEGRAM_BOT_TOKEN || '';
      const TELEGRAM_CHAT_ID = process.env.REACT_APP_TELEGRAM_CHAT_ID || '';

      const response = await fetch(
        `https://api.telegram.org/bot${TELEGRAM_BOT_TOKEN}/sendMessage`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            chat_id: TELEGRAM_CHAT_ID,
            text: message,
            parse_mode: 'Markdown',
            disable_notification: false,
          }),
        }
      );

      return response.ok;

    } catch (error) {
      console.error('Erro Telegram:', error);
      return false;
    }
  };

  // Formatar duração (mm:ss)
  const formatDurationFromSeconds = (seconds: number): string => {
    const mins = Math.floor(seconds / 60);
    const secs = seconds % 60;
    return `${mins.toString().padStart(2, '0')}:${secs.toString().padStart(2, '0')}`;
  };

  return (
    <div className="min-h-screen bg-gradient-to-b from-orange-900/20 to-black">
      <div className="container mx-auto px-4 py-8 max-w-4xl">
        
        <div className="mb-10">
          <button
            onClick={() => navigate('/academia')}
            className="mb-6 inline-flex items-center gap-2 text-orange-300 hover:text-white transition-colors"
          >
            <i className="fas fa-arrow-left"></i>
            Voltar
          </button>
          
          <div className="flex items-center gap-4 mb-6">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-orange-500 to-yellow-500 
              flex items-center justify-center">
              <i className="fas fa-person-walking text-2xl text-white"></i>
            </div>
            <div>
              <h1 className="text-3xl font-bold text-white">Esteira / Caminhada</h1>
              <p className="text-orange-300">Registro de atividade aeróbica</p>
            </div>
          </div>
        </div>

        <div className="bg-gradient-to-br from-secondary-dark/50 to-black/50 rounded-2xl p-6 
          border border-white/10 mb-8">
          
          <h2 className="text-xl font-bold text-white mb-6">📋 Dados do Treino</h2>
          
          <div className="mb-4">
            <label className="block text-white mb-2">
              Duração (minutos) *
            </label>
            <div className="flex flex-wrap gap-2 mb-2">
              {quickDurations.map((dur) => (
                <button
                  key={dur}
                  type="button"
                  onClick={() => setDuration(dur.toString())}
                  className={`px-3 py-2 rounded-lg transition-all ${
                    duration === dur.toString()
                      ? 'bg-orange-600 text-white'
                      : 'bg-white/5 text-white hover:bg-white/10'
                  }`}
                >
                  {dur}min
                </button>
              ))}
            </div>
            <input
              type="number"
              value={duration}
              onChange={(e) => setDuration(e.target.value)}
              placeholder="Ex: 30"
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 
                text-white placeholder-gray-400 focus:outline-none focus:border-orange-500"
              min="1"
              max="240"
              step="0.1"
              required
            />
          </div>

          <div className="mb-4">
            <label className="block text-white mb-2">
              Distância percorrida (metros) *
            </label>
            <div className="flex flex-wrap gap-2 mb-2">
              {quickDistances.map((dist) => (
                <button
                  key={dist}
                  type="button"
                  onClick={() => setDistance(dist.toString())}
                  className={`px-3 py-2 rounded-lg transition-all ${
                    distance === dist.toString()
                      ? 'bg-orange-600 text-white'
                      : 'bg-white/5 text-white hover:bg-white/10'
                  }`}
                >
                  {dist/1000}km
                </button>
              ))}
            </div>
            <input
              type="number"
              value={distance}
              onChange={(e) => setDistance(e.target.value)}
              placeholder="Ex: 5000"
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 
                text-white placeholder-gray-400 focus:outline-none focus:border-orange-500"
              min="0"
              max="50000"
              step="100"
              required
            />
          </div>

          <div className="mb-4">
            <label className="block text-white mb-2">
              Frequência cardíaca média (bpm) *
            </label>
            <input
              type="number"
              value={heartRate}
              onChange={(e) => setHeartRate(e.target.value)}
              placeholder="Ex: 137"
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 
                text-white placeholder-gray-400 focus:outline-none focus:border-red-500"
              min="40"
              max="220"
              required
            />
          </div>

          <div className="mb-6">
            <label className="block text-white mb-2">
              Calorias perdidas (kcal) *
            </label>
            <input
              type="number"
              value={calories}
              onChange={(e) => setCalories(e.target.value)}
              placeholder="Ex: 252"
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 
                text-white placeholder-gray-400 focus:outline-none focus:border-green-500"
              min="0"
              max="2000"
              required
            />
          </div>

          {/* Cards de Estatísticas em Tempo Real */}
          {distance && duration && (
            <div className="grid grid-cols-2 gap-4 mb-6">
              <div className="bg-orange-500/10 rounded-xl p-4 border border-orange-500/20">
                <p className="text-orange-300 text-xs mb-1">Velocidade Média</p>
                <p className="text-white font-bold text-xl">
                  {calculateAvgSpeed().toFixed(1)} km/h
                </p>
              </div>
              <div className="bg-orange-500/10 rounded-xl p-4 border border-orange-500/20">
                <p className="text-orange-300 text-xs mb-1">Pace</p>
                <p className="text-white font-bold text-xl">
                  {calculatePace().toFixed(2)} min/km
                </p>
              </div>
            </div>
          )}

          <div className="mb-8">
            <label className="block text-white mb-2">
              Observações (opcional)
            </label>
            <textarea
              value={notes}
              onChange={(e) => setNotes(e.target.value)}
              placeholder="Ex: Sensação de cansaço, boa resistência..."
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 
                text-white placeholder-gray-400 focus:outline-none focus:border-blue-500 
                resize-none h-24"
              maxLength={300}
            />
          </div>

          <div className="flex gap-3">
            <button
              onClick={() => navigate('/academia')}
              className="flex-1 py-3 bg-white/10 text-white rounded-xl hover:bg-white/20 transition-colors"
              disabled={isLoading}
            >
              Cancelar
            </button>
            <button
              onClick={handleSubmit}
              disabled={isLoading || !duration || !heartRate || !calories || !distance}
              className={`flex-1 py-3 rounded-xl font-bold transition-all ${
                !isLoading && duration && heartRate && calories && distance
                  ? 'bg-gradient-to-r from-orange-600 to-yellow-600 text-white hover:opacity-90'
                  : 'bg-gray-700 text-gray-400 cursor-not-allowed'
              }`}
            >
              {isLoading ? 'Enviando...' : 'Registrar Treino'}
            </button>
          </div>
        </div>

        {/* Preview dos dados que serão salvos */}
        {duration && distance && heartRate && calories && (
          <div className="bg-orange-500/10 rounded-2xl p-6 border border-orange-500/20">
            <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
              <i className="fas fa-eye"></i>
              Preview do Registro
            </h3>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-text-secondary">Duração:</span>
                <span className="text-white">{duration} min</span>
              </div>
              <div className="flex justify-between">
                <span className="text-text-secondary">Distância:</span>
                <span className="text-white">{(parseInt(distance) / 1000).toFixed(2)} km</span>
              </div>
              <div className="flex justify-between">
                <span className="text-text-secondary">Velocidade Média:</span>
                <span className="text-accent-green font-bold">{calculateAvgSpeed().toFixed(1)} km/h</span>
              </div>
              <div className="flex justify-between">
                <span className="text-text-secondary">Pace:</span>
                <span className="text-white">{calculatePace().toFixed(2)} min/km</span>
              </div>
              <div className="flex justify-between">
                <span className="text-text-secondary">FC Média:</span>
                <span className="text-accent-blue">{heartRate} bpm</span>
              </div>
              <div className="flex justify-between">
                <span className="text-text-secondary">Calorias:</span>
                <span className="text-accent-green">{calories} kcal</span>
              </div>
            </div>
          </div>
        )}
      </div>
    </div>
  );
};

export default EsteiraRegistration;