// src/pages/Workout2/Workout2.tsx
...
const Workout2: React.FC = () => {
  ...
  useEffect(() => {
    console.log('📊 Workout2 - Estado atual:', {
      isInitialized,
  ...
  const loadLastWeights = useCallback(async () => {
    try {
      ...
      const exercisesList = workouts3DaysData.workout2.exercises || [];
      
      for (const exercise of exercisesList) {
        if (exercise.noWeight || exercise.isCardio) continue;
        const lastWeight = await workoutService.getLastExerciseWeight(exercise.id, '2');
        if (lastWeight) {
  ...
  const loadLastRecords = useCallback(async () => {
    try {
      ...
      for (const exercise of exercisesList) {
        if (exercise.noWeight || exercise.isCardio) continue;
        const lastRecord = await workoutService.getLastExerciseRecord(exercise.id, '2');
        if (lastRecord) {
  ...
  useEffect(() => {
    console.log('🚀 Workout2 - Iniciando inicialização...');
    const workoutData = workouts3DaysData.workout2;
    
    if (workoutData && workoutData.exercises.length > 0) {
      console.log('✅ Workout2 - Dados encontrados:', workoutData.exercises.length, 'exercícios');
      ...
      console.error('❌ Workout2 - Dados não encontrados!');
    }
  ...
  useEffect(() => {
    if (session.workoutStarted) {
      ...
      console.log('🔄 Workout2 - Opções de exercícios atualizadas:', options.length);
    }
  ...
  const finalizeWorkoutManually = () => {
    localStorage.removeItem('workout-2');
    navigate('/');
  };
  ...
  const handleFinalizeWithCalories = async (calories: number, heartRate?: number) => {
    try {
      ...
      console.log('🔥 FINALIZANDO TREINO 2:', {
        durationInSeconds,
      ...
      await workoutService.save({
        ...
        notes: 'Treino 2 finalizado - Costas/Trapézio/Bíceps/Antebraço + Abdominal + Caminhada'
      });
    ...
    return (
      <div className="min-h-screen bg-primary-dark flex items-center justify-center">
        <div className="text-center">
          ...
          <h2 className="text-xl font-bold text-white mb-2">Carregando Treino 2...</h2>
      ...
      <div className="min-h-screen bg-primary-dark">
        <div className="container mx-auto px-4 py-6 max-w-6xl">
          ...
          <header className="bg-gradient-to-r from-secondary-dark to-black rounded-2xl p-6 mb-8 
            ...
            >
              <i className="fas fa-arrow-left text-blue-400 group-hover:translate-x-[-3px] transition-transform"></i>
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-text-secondary uppercase tracking-wider text-sm font-inter">
                  Costas • Trapézio • Bíceps • Antebraço • Abdominal • Caminhada
                </p>
              </div>
            </div>
          ...
          <main className="bg-gradient-to-b from-secondary-dark/50 to-transparent 
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
              ...
              <div className="bg-gradient-to-br from-secondary-dark to-black rounded-xl p-6 
                border border-white/5 text-center">
                <i className="fas fa-fire text-4xl text-blue-400 mb-4"></i>
                <h3 className="text-4xl font-bold text-white mb-2">
                  {totalSets}
                </h3>
                <p className="text-text-secondary uppercase tracking-wider text-sm">
          ...
          <footer className="bg-gradient-to-r from-secondary-dark to-black rounded-2xl p-8 
            ...
            <div className="flex items-center gap-4 text-text-secondary">
              <i className="fas fa-info-circle text-xl text-blue-400"></i>
              <div>
